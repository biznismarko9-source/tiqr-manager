    ...
    <Modal open={open} onClose={onClose} title="New order" width="max-w-2xl">
      <div className="flex flex-col gap-4">
        {/* 2.7.0: AI Import Assistant. Every line below sets one of THIS
            ...
            no second create path, no direct api.createOrder call, and the
            Create button, its validation and insert_order_with_tickets are
            all untouched. Ticket-level values come from the one group marko
            picked in the panel (an order carries one section/row/tier/price,
            so a second group is a second order - the panel says so). */}
        <AiImportPanel
          kind="order"
          onApply={({ fields, group }) => {
            ...
            // The order code is generated by the backend (ORD-000001), so an
            // extracted marketplace reference has no column of its own -
            // notes is where it goes rather than being dropped. Appended,
            // never overwriting whatever is already typed there.
            ...
            if (group) {
              ...
              if (group.unitPrice) setUnitPrice(group.unitPrice);
              if (group.fees) setUnitFees(group.fees);
              if (group.tier) setTier(group.tier);
              if (group.section) setSection(group.section);
              if (group.row) setRowLabel(group.row);
              // parseSeats already understands a comma-separated list, and
              // the panel hands over one label per ticket - so this is the
              // existing seats mechanism, not a new one.
              if (group.seats.length > 0) setSeatsRaw(group.seats.join(", "));
              const extractedType = group.ticketType?.trim();
              if (extractedType) {
                // An unrecognised type flips the field into its existing
                // free-text mode rather than being silently discarded or
                // added to the shared lookup list.
                const known = ticketTypeOptions.find(
                  (t) => t.toLowerCase() === extractedType.toLowerCase(),
                );
                setCustomTicketType(!known);
                setTicketType(known ?? extractedType);
              }
            }
